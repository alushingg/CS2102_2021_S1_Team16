...
exports.getPets = function(req, type, sdate, edate, callback) {
    ...
    const query = "SELECT name FROM own_pet_belong WHERE username = '" + req.session.user.username + "' AND type = '" + type + "'; "
...
// WHERE username = $username;
    ...
    dbController.queryGet(pricequery, (result) => {
        if (result.status == 200) {
            ...
            const query = period + "INSERT INTO take_care VALUES ('" + req.session.user.username + "', '" + req.body.petname + "', '" + sdate +
